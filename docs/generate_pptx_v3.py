"""
Генератор PPTX-презентации ВКР — Recall (v3 - clean modern)
Запуск: source .venv/bin/activate && python docs/generate_pptx_v3.py
Результат: docs/ВКР_Миняев_МИ_v3.pptx
"""
...
def create_presentation():
    ...
    for txt, sz, clr, bld, ital in [
        ("f", Pt(15), BLACK, False, True),
        ("img", Pt(9), GRAY, False, False),
        (" : Image \u2192 \u211d", Pt(14), BLACK, False, False),
        ("d", Pt(9), BLACK, False, False),
        ("              ", Pt(14), BLACK, False, False),
        ("f", Pt(15), BLACK, False, True),
        ("text", Pt(9), GRAY, False, False),
        (" : Text \u2192 \u211d", Pt(14), BLACK, False, False),
        ("d", Pt(9), BLACK, False, False),
        ("              ", Pt(14), BLACK, False, False),
    ...
    for txt, sz, clr, bld in [
        ("sim(q\u0302, v\u0302", Pt(15), BLACK, False),
        ("i", Pt(9), BLACK, False),
        (") = q\u0302 \u00b7 v\u0302", Pt(15), BLACK, False),
        ("i", Pt(9), BLACK, False),
        ("              ", Pt(14), BLACK, False),
    ...
    tools = [
        ("Kotlin 2.2", "типобезопасность, корутины"),
        ("KMP", "единая логика Android + iOS"),
        ("Jetpack Compose", "декларативный UI, Material 3"),
        ("Room 2.8", "типобезопасная ORM для SQLite"),
        ("TensorFlow Lite", "инференс CLIP на NPU/GPU"),
        ("HNSW (Kotlin)", "векторный поиск O(log n)"),
        ("WorkManager", "фоновая индексация + батарея"),
        ("Hilt", "внедрение зависимостей"),
        ("MVI", "однонаправленный поток данных"),
        ("kotlin.test", "единые тесты для обеих платформ"),
        ("Android Studio", "IDE + Profiler (Memory, Energy)"),
        ("Git", "контроль версий"),
    ]
    for i, (name, why) in enumerate(tools):
        ...
        y = Cm(3.2) + row * Cm(2.2)
        add_card(slide, x, y, Cm(11.5), Cm(1.9))
        txBox = slide.shapes.add_textbox(x + Cm(0.7), y + Cm(0.45), Cm(10.3), Cm(1.1))
        tf = txBox.text_frame
        tf.word_wrap = True
        p = tf.paragraphs[0]
        r = p.add_run()
    ...
    func_items = [
        "Семантический поиск", "Фоновая индексация",
    ...
    # Save
    out = os.path.join(os.path.dirname(__file__), "ВКР_Миняев_МИ_v3.pptx")
    prs.save(out)
